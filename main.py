...
Date: 26th May 2022
Brief Project Description: A reading tracker which allows readers to jeeo track of the books we read and what page we stopped for easier convinence for when we pick it up again.
GitHub URL: https://github.com/JCUS-CP1404/assignment-2-Rockhp.git
"""
...
class MainScreen(Screen):  # This class is pops up GUI the main screen
...
class BookLabel(Label):  # Base Label Class
...
class HeadLabel(BookLabel):  # Top inscription
    ...
    def set_label_text(self):  # Displays the number of pages required to read
        self.text = 'Pages to read: {}'.format(self.collection.get_required_pages())


class BookButton(Button):  # A button with a link to a specific book
    ...
    def set_color(self):  # Setting the button color depending on the need / completion of reading
        self.background_color = 'white' if self.book.is_completed else 'aqua'

    def on_press(self):  # Handling a book button click
...
class MainBox(BoxLayout):  # Base page layout class and links to parts of the page layout that are set in the kv file
...
class WarningLabel(BookLabel):  # Bottom information label
